...
from grok_x_lead_monitor.exporter import (
    HEADER,
    SEPARATOR,
    render_json_records,
...
def test_write_json_report_writes_sorted_rows(tmp_path: Path):
    leads = [
        LeadRecord(
            username="low",
            pain_point_tag="Foot Pain",
    ...
    output_path = write_json_report(tmp_path, "2026-04-08", leads)
    content = json.loads(output_path.read_text())

    assert output_path == tmp_path / "2026-04-08.json"
    assert [record["username"] for record in content] == ["high", "low"]
    assert content[0]["tweet_created_at"] == "2026-04-08T02:00:00+00:00"


def test_render_json_records_uses_deterministic_tie_breakers():
    leads = [
        LeadRecord(
            username="zeta",
            pain_point_tag="Foot Pain",
    ...
    records = render_json_records(leads)

    assert [record["username"] for record in records] == ["alpha", "zeta"]
